...
class Array(Cable):
    ...
    def calc_max_turb_per_cable(self, addl_inputs):
        ...
        Parameters
        ----------
        turbine_ampacity : int
            Amperage generated by each turbine at the collection system voltage                                                    
        turbine_rating_MW : int
        OR
            Nameplate capacity of individual turbines
        """
        turbine_rating_MW = addl_inputs['turbine_rating_MW']
        self.max_turb_per_cable = np.floor(self.current_capacity / addl_inputs['turbine_ampacity'])
        # self.max_turb_per_cable = np.floor(self.cable_power / turbine_rating_MW) #TODO figure out if you want to calc with power rating or ampacity!!!
    ...
    def calc_num_turb_per_cable(self, addl_inputs):
        ...
        self.num_turb_per_cable = self.max_turb_per_cable - upstream_turb  # todo: add to ouptut csv
    ...
    def calc_array_cable_len(self, addl_inputs):
        ...
        self.array_cable_len = ((self.num_turb_per_cable + self.downstream_connection) * self.turb_section_length)  # todo: add to output csv
...
class ArraySystem(CostModule):
    ...
    * Given below are attributes that define each cable type:
    ...
    def calc_current_properties(self):
        """
        Find collection system voltage [V] and turbine amperage [A]. Sort cables by current capacity.

        Returns
        -------
        self.collection_V: collection system voltage [V]
        self.turbine_ampacity: turbine amperage [A] at collection system voltage
        """
        ...
        self.turbine_ampacity = self.input_dict['turbine_rating_MW']*1e3/self.collection_V

    def calc_required_segment_ampacity(self):
        """
        Find the type of cable needed for each cable segment based on ampacity needed
        ...
        giver = np.where(self.C > 0)[0]  # nodes that give ampacity to downstream nodes
        ...
        while np.prod(self.C[1:]) == 0:  # run until all turbine nodes are assigned a capacity
            ...
            for i in np.setdiff1d(x, y):  # iterate through all nongiver, nonclosed nodes
                ...
                if a == b:  # if all but one receiver node are givers
                    self.C[i] += sum(self.C[receiver]) + 1  # add giver node capacities to current node. +1 for this node's turbine
        ...
        self.C *= self.turbine_ampacity     # scale ampacity by turbine current

        """
        Create cable dictionary with cable start point, end point, length, ampacity, and bool representing if the cable segment is the terminal
        """
        k = 0  # cable# iterator
        ...
        for i in range(0, self.n_segments):
            for j in np.where(self.A[i, :] * remains == 1)[0]:
                ...
                array_dict['cable' + str(k)]['Ampacity'] = min(self.C[i], self.C[j])
                array_dict['cable' + str(k)]['Terminal?'] = False
                k += 1  # iterate to make new cable
            remains[i] = False  # prevent duplicate cables
        # add terminal cable
        array_dict['cable' + str(k)] = dict()
        array_dict['cable' + str(k)]['Length'] = self.input_dict['distance_to_grid_connection_mi'] * 5280 * self._km_to_LF
        array_dict['cable' + str(k)]['Ampacity'] = self.n_segments*self.turbine_ampacity
    ...
    def calc_num_strings(self):
        ...
        self.output_dict['num_full_strings'] = np.floor(self.output_dict['total_turb'] / self.output_dict['total_turb_per_string'])  # todo: add to output csv
        ...
        # todo: output number of partial strings
    ...
    def calc_cable_len_to_substation(self, turbine_spacing_rotor_diameters, row_spacing_rotor_diameters,
                                     num_strings):
        """
 ...
 # If terminal cable has already been accounted for, skip any calculations for other cables.
        ...
        if num_full_strings <= 1 and num_partial_strings >= 0:   # Essentially a switch for distributed wind

            # if number of turbines is less than total string capacity, find the terminal cable and find total cable len
            # up till that cable.
            if total_turbines <= turbines_per_cable[count]: # If total turbines in project are less than cumulative turbines uptil and including that cable.
                # total_cable_len = (num_full_strings * cable.array_cable_len + num_partial_strings * (cable.array_cable_len * perc_partial_string) + len_to_substation)
                ...
                if (cable.turb_sequence - 1 ) == 0: # That is, if cable #1 can hold more turbines than specified by user
                    ...
                    total_cable_len = ((num_full_strings * cable.array_cable_len) + (num_partial_strings * cable.array_cable_len)) + len_to_substation
    ...
    def create_ArraySystem(self):
        ...
        self.addl_specs['turbine_ampacity'] = self.turbine_ampacity
        system = {
        ...
        if self.input_dict['user_defined_home_run_trench'] == 0:                                                                                                                                                                                                        
            distributed_wind_distance_to_grid = (self.input_dict['turbine_spacing_rotor_diameters'] * self.input_dict['rotor_diameter_m']) / 1000       #This only gets used if number of strings is <= 1
            self.output_dict['trench_len_to_substation_km'] = self.calc_cable_len_to_substation(self.input_dict['turbine_spacing_rotor_diameters'], self.input_dict['row_spacing_rotor_diameters'], self.output_dict['num_strings'])
        else:
            self.output_dict['trench_len_to_substation_km'] = self.input_dict['trench_len_to_substation_km']

        self.output_dict['cable_len_to_substation_km'] = self.output_dict['trench_len_to_substation_km'] # assumes 3 conductors and fiber and neutral
        ...
        for idx, (name, cable) in enumerate(self.cables.items()):
            ...
            pps = self.output_dict['perc_partial_string'][idx]
            total_cable_len, self.check_terminal = self.calc_total_cable_length(self.output_dict['total_turb'], idx,
                                                            self.check_terminal, self.__turbines_on_cable,
                                                            cable, self.input_dict['cable_specs'],
                                                            self.output_dict['num_full_strings'],
                                                            self.output_dict['num_partial_strings'],
                                                            self.output_dict['cable_len_to_substation_km'],
                                                            pps,
                                                            )
            ...
            # cable.total_mass = total_cable_len * cable.mass
    ...
    def create_manual_ArraySystem(self):
        ...
        self.calc_required_segment_ampacity()
        ...
        for segment in self.array_dict:
            for idx, (name, cable) in enumerate(self.cables.items()):
                if cable.current_capacity >= self.array_dict[segment]['Ampacity']:
                    ...
                    dissipated_power += 3 * self.array_dict[segment]['Ampacity']**2 * abs(cable.ac_resistance)*self.array_dict[segment]['Length']/1000    #TODO P=3*I^2*R. IF 3 phase. Divide by 1000 to go from ohm/km->ohm/m
    ...
    def calculate_costs(self, calculate_costs_input_dict, calculate_costs_output_dict):
        ...
        trenching_equipment_rental_cost_df = pd.DataFrame([['Equipment rental',calculate_costs_output_dict['Equipment Cost USD with weather delays'], 'Collection']],
        ...
        if calculate_costs_input_dict['num_turbines'] > 10:
            ...
            calculate_costs_output_dict['mob_cost'] = (collection_cost['Cost USD'].sum() / calculate_costs_input_dict['num_turbines']) * self.mobilization_cost(calculate_costs_input_dict['turbine_rating_MW'])
        ...
        calculate_costs_output_dict['sum_collection_cost'] = collection_cost['Cost USD'].sum()                                                                                              
